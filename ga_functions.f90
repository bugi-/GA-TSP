...
module ga_functions
  ...
  function gen_new_generation(old_pop, positions) result(population)
    ...
    do i = 1, pop_size
      ! Get a partner for each route by generating a random integer, which is not same is i
      mate = rand_int(pop_size - 1)
      if (mate >= i) mate = mate + 1
      population(i,:) = create_child(old_pop(i,:), old_pop(mate,:), positions)
    end do
  end function
